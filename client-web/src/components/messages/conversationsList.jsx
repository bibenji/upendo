				...
				<div className="list-group">
                    <div className="list-group-item list-group-item-action flex-column align-items-start active">
                        <div className="d-flex w-100 justify-content-between">
                            ...
                            <p className="mb-0">Dernier message</p>
                        </div>
					...
					{this.props.conversations.map((conv, index) => {
						...
						return (
							<Link key={index} to={`${this.props.match.url}/${conv.id}`} className="list-group-item list-group-item-action d-flex flex-row align-items-stretch justify-content-between">
								<div className="d-flex w-25 flex-row justify-content-center">
									{conv.users.map((user, index) => {
										if (user.id !== this.props.connectedUserId) {
											return (
												<div key={index} className="text-md-center p-1">
													<img src={user.photos[0] ? "http://assets.upendo.localhost"+user.photos[0].path : "https://unsplash.it/200/200"} className="rounded mb-1" width="50px" height="50px" /><br />
													<h5 className="conversations_list-contact-name">
														{user.username}
													</h5>
												</div>
											);
								...
								{lastMessage ?
									<div className="d-flex w-70 flex-column justify-content-center text-md-right">
